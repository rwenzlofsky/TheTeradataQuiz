package com.dwhpro.countdownanimation;


import com.dwhpro.quizappnew.R;
import com.dwhpro.utils.Utils;
...
public class CountDownProgress extends View{
	...
	private final int default_finished_color = Color.rgb(0, 255, 0);
	...
	public CountDownProgress(Context context) {
		...
		Log.d("contextCountDown", String.valueOf(context));
	}
	...
	public CountDownProgress(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		//HAConstant haConstant =  (HAConstant) context;
      //  Toast.makeText(context, haConstant.getDuration(), Toast.LENGTH_LONG).show();
		final TypedArray attributes = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CountDownProgress, defStyleAttr, 0);
	...
	protected void initByAttributes(TypedArray attributes) {
		finishedStrokeColor = attributes.getColor(R.styleable.CountDownProgress_donut_finished_color, default_finished_color);
		unfinishedStrokeColor = attributes.getColor(R.styleable.CountDownProgress_donut_unfinished_color, default_unfinished_color);
		textColor = attributes.getColor(R.styleable.CountDownProgress_donut_text_color, default_text_color);
		textSize = attributes.getDimension(R.styleable.CountDownProgress_donut_text_size, default_text_size);

		setMax(attributes.getInt(R.styleable.CountDownProgress_donut_max, default_max));
		setProgress(attributes.getInt(R.styleable.CountDownProgress_donut_progress, 0));
		finishedStrokeWidth = attributes.getDimension(R.styleable.CountDownProgress_donut_finished_stroke_width, default_stroke_width);
		unfinishedStrokeWidth = attributes.getDimension(R.styleable.CountDownProgress_donut_unfinished_stroke_width, default_stroke_width);
		if (attributes.getString(R.styleable.CountDownProgress_donut_prefix_text) != null) {
			prefixText = attributes.getString(R.styleable.CountDownProgress_donut_prefix_text);
		}
		if (attributes.getString(R.styleable.CountDownProgress_donut_suffix_text) != null) {
			// suffixText = attributes.getString(R.styleable.CountDownProgress_donut_suffix_text);
		}
		if (attributes.getString(R.styleable.CountDownProgress_donut_text) != null) {
			text = attributes.getString(R.styleable.CountDownProgress_donut_text);
		}
		innerBackgroundColor = attributes.getColor(R.styleable.CountDownProgress_donut_background_color, default_inner_background_color);
		innerBottomTextSize = attributes.getDimension(R.styleable.CountDownProgress_donut_inner_bottom_text_size, default_inner_bottom_text_size);
		innerBottomTextColor = attributes.getColor(R.styleable.CountDownProgress_donut_inner_bottom_text_color, default_inner_bottom_text_color);
		innerBottomText = attributes.getString(R.styleable.CountDownProgress_donut_inner_bottom_text);

		startingDegree = attributes.getInt(R.styleable.CountDownProgress_donut_circle_starting_degree, default_startingDegree);
	}
